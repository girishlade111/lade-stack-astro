...
for label, rel in pages.items():
    ...
    def safe(s):
        return htmllib.unescape(s).encode("ascii", "backslashreplace").decode("ascii")
    print(f"--- dist/{label} ---")
    print("  title:", safe(title.group(1)) if title else "MISSING")
    print("  meta description:", safe(desc.group(1)[:120]) + "..." if desc else "MISSING")
    ...
    print("  pills aria-label:", safe(pills.group(1)) if pills else "MISSING")
    one = re.search(r'data-count-one="([^"]*)"', raw)
    other = re.search(r'data-count-other="([^"]*)"', raw)
    print("  count-one:", safe(one.group(1)) if one else "MISSING")
    print("  count-other:", safe(other.group(1)) if other else "MISSING")
