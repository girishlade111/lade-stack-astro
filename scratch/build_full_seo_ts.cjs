...
// Load verified CJK descriptions
const cjkCode = fs.readFileSync('scratch/verify_cjk_final.cjs', 'utf8');
const fn = new Function(`${cjkCode}; return { zhDesc, koDesc, jaDesc };`);
const { zhDesc, koDesc, jaDesc } = fn();
...
for (const [loc, pages] of Object.entries(seoData)) {
  for (const [pk, p] of Object.entries(pages)) {
    ...
    const minT = isCJK ? 30 : 45;
    if (tLen < minT || tLen > 60) {
      console.error(`FAIL [${loc}][${pk}] Title len ${tLen} out of [${minT}, 60]: "${p.title}"`);
      failures++;
    }
    if (dLen < 140 || dLen > 160) {
      console.error(`FAIL [${loc}][${pk}] Desc len ${dLen} out of [140, 160]: "${p.description}"`);
      failures++;
    }
    const kwArr = p.keywords.split(',').map(s => s.trim());
    if (kwArr.length < 4) {
      console.error(`FAIL [${loc}][${pk}] Keywords count ${kwArr.length} < 4`);
      failures++;
    }
  }
}

console.log(`Global audit completed. Total failures: ${failures}`);
if (failures === 0) {
  // Generate TypeScript code
  let tsCode = `import type { SupportedLocale } from './config';
...
export const seoData: Record<SupportedLocale, Record<SEOPageKey, PageSEO>> = ${JSON.stringify(seoData, null, 2)};
...
`;
  ...
  fs.writeFileSync('src/i18n/seo.ts', tsCode, 'utf8');
  console.log('Successfully wrote src/i18n/seo.ts!');
}
