...
export const zh: LocaleContent = {
  ...
  supportTopics: [
    {
      title: '快速上手',
      text: '从应用画廊中即可直接启动任意工具，无需强制注册、付费墙或安装依赖。核心实用工具完全在浏览器客户端本地运行，让您的工作流保持即时、私密且轻便便携。'
    },
    {
      title: '文件工具',
      text: 'LS PDF Tools 和 LS Image Studio 等工具利用 WebAssembly 和 Canvas 在浏览器本地内存中处理文件，文件绝不会上传至外部服务器。对于大文件，建议使用可用内存充足的现代桌面浏览器。'
    },
    {
      title: 'CodeEnhance AI',
      text: '粘贴代码片段即可分析结构、重构语法并获取实时 AI 辅助建议。请将 AI 产出视为结对编程的参考助手：在部署到生产环境之前，务必人工审查、静态检查并运行测试以验证安全关键逻辑。'
    },
    {
      title: '账号与数据',
      text: '零账号绑定。您的工作会话、草稿和自定义偏好均保存在浏览器本地存储（LocalStorage/IndexedDB）中。建议定期利用内置的导出功能，将重要文件、配置和优化后的代码下载到本地电脑。'
    }
  ],
  supportFaqs: [
    {
      q: '工具打不开或无响应怎么办？',
      a: '首先请尝试强制刷新页面（Ctrl+F5 或 Cmd+Shift+R）以清除过期的缓存脚本。接着检查是否有激进的广告拦截器或安全脚本扩展阻止了 ladestack.in 的资源请求。您可以在无痕/隐身窗口中打开以排查扩展冲突。若问题仍未解决，可按 F12 打开开发者工具控制台，排查 WebAssembly 或网络资源加载错误，并附带浏览器版本与错误日志通过联系页面向我们反馈。'
    },
    {
      q: '我的文件、文档或代码会被上传到外部服务器吗？',
      a: '不会。对于 LS PDF Tools 和 LS Image Studio 等客户端文件工具，文件解析、图片处理和格式转换完全在您浏览器的本地内存中通过 WebAssembly 和 HTML5 Canvas API 执行，文件绝不会上传至我们的服务器或在云端持久保存。对于 CodeEnhance AI 等 AI 工具，输入的代码仅安全传输到临时内存用于生成建议，绝不会写入磁盘或用于训练第三方模型。'
    },
    {
      q: '如何高效地提交一份 Bug 报告？',
      a: '一份清晰的报告通常包含：工具名称及页面 URL、您的操作系统与浏览器版本、触发该 Bug 的精确重现步骤、实际结果与预期结果的对比，以及浏览器控制台（F12）的报错日志。截图或屏幕录像将极大加快排查速度。您可以通过联系页面（/contact）或发送邮件至 admin@ladestack.in 提交，清晰可复现的报告能帮助我们快速发布修复补丁。'
    },
    {
      q: '我可以提议新增工具或申请新功能吗？',
      a: '当然可以，社区开发者的真实需求直接指引着 Lade Stack 的研发路线图。我们非常欢迎有关开发者工具、效率提升利器和 AI 工作流的新构想。我们会从实际开发价值、架构可行性、性能开销以及对轻量化客户端哲学的契合度进行综合评估。欢迎通过联系页面或 GitHub Discussions 分享您的想法。'
    },
    {
      q: '免费工具有正式的 SLA（服务等级协议）保障吗？',
      a: '我们不针对免费工具提供企业级的合同 SLA 或商业可用性保证。不过，我们的整体架构基于静态预渲染、全球 CDN 边缘分发以及不间断的运行监控，目标保持 99.9% 的高可用性。对于功能缺陷或浏览器兼容性问题，我们会列为高优先级并在第一时间全力修复。'
    },
    {
      q: '在哪里可以学习最佳实践并查阅技术文档？',
      a: '您可以查阅我们的文档中心（/docs），获取架构说明、集成指南以及 CodeEnhance AI 等工具的分步教程。此外，我们的工程博客（/blog）还收录了 27 篇以上深度实战文章，涵盖生成式 AI 工程化、前端极致性能、客户端数据隐私以及现代 Web 开发技术。'
    },
    {
      q: '如果 CodeEnhance AI 给出的结果不符合预期或不完整该怎么办？',
      a: '首先检查输入代码：建议拆解为具体的独立函数或组件，并附带明确的语言声明与简洁上下文。过于庞大且未经整理的代码块可能分散模型的注意力。若结果仍不够理想，可尝试细化需求指令或采用分步增量的方式提问。请注意 AI 生成的是概率性建议，在合入生产分支前必须进行人工审查、格式化和单元测试。'
    },
    {
      q: '如果文件转换或下载失败该如何处理？',
      a: '浏览器端的文件处理直接依赖您设备当前的可用内存。如果处理中断或卡住，请先关闭其他高内存占用的浏览器标签页以释放 RAM，并确保浏览器已允许 ladestack.in 自动下载文件。对于超大体积的 PDF 或超高分辨率图片，建议分批次拆分处理，以避免触发浏览器标签页崩溃。'
    },
    {
      q: '使用 Lade Stack 需要注册账号吗？',
      a: '不需要。Lade Stack 平台上的所有核心开发者工具和媒体处理工具均可直接使用，无需注册账号、无需验证邮箱，也无需绑定信用卡。我们坚信实用的工具应当人人唾手可得、零门槛开放。直接在应用画廊中打开工具即可立即投入工作。'
    },
    {
      q: '我的会话数据如何保存？清除网站数据后会发生什么？',
      a: '所有工作状态、临时草稿和自定义偏好均保存在您本地设备的浏览器 LocalStorage 和 IndexedDB 中。如果您清除了浏览器缓存或网站数据，这些本地配置将会被重置。为了防止意外丢失进度，请在处理完成后第一时间将导出的成果文件、配置文件或代码保存到本地硬盘。'
    },
    {
      q: '团队、初创企业和高校师生可以免费使用 Lade Stack 吗？',
      a: '可以。Lade Stack 工具完全免费开放给商业机构、初创团队、个人开发者和学术教育场景使用。我们的免费核心目录不存在席位限制、隐藏功能锁或额外的团队商业许可要求，工程团队可以无拘无束地将其纳入日常开发工作流。'
    },
    {
      q: '支持团队的回复时效是多久？如何联系创始人或团队？',
      a: '我们承诺在 24 小时内回复所有技术咨询、Bug 反馈及合作提议。您可以通过联系页面（/contact）提交表单，或直接致信 admin@ladestack.in。提供详尽的运行环境信息和重现步骤将帮助我们以最快速度为您提供切实可行的技术解答。'
    }
  ],
  contactFaqs: [
    {
    ...
    },
    {
      ...
      a: 'Lade Stack 由 Girish Lade 创立并担任架构设计，其角色为创始人兼首席软件架构师。Girish 融合 UI/UX 设计、前后端工程、云原生架构与生成式 AI 的跨领域背景，把控产品方向并亲自打造解决实际工程痛点的实用工具。'
    },
    {
  ...
  privacy: [
    {
      h: '概述与隐私原则',
      p: 'Lade Stack 是由 Girish Lade 在印度创立的独立开发者工具与生成式 AI 工程生态。我们最核心的设计原则是“源头隐私”：软件应当赋能用户，而非通过侵入式遥测、无端监控或隐蔽的数据变现来牟利。我们在技术可行范围内严格将数据收集降至最低，尽量将文件处理与计算保留在用户的本地浏览器中执行，绝不向数据经纪商或广告网络出售、出租您的个人信息。'
    },
    {
      h: '您主动提供的信息',
      p: '我们仅在您主动提交时收集个人信息。当您通过联系页面（/contact）发送留言时，需填写姓名、电子邮箱及留言内容，这些信息通过 Formspree 安全地投递至我们的工程团队收件箱，以便我们及时答复。如果您在网站底部订阅技术动态与通讯（Newsletter），我们收集您的邮箱地址仅用于发送更新通知和开发教程。若遇到技术支持需求、功能建议或错误反馈，您亦可随时直接发送邮件至 admin@ladestack.in。'
    },
    {
      h: '自动收集的技术数据与分析',
      p: '为保障网站运行稳定性、排查路由故障并了解整体使用趋势，我们采用了注重隐私的数据度量工具。我们使用 Umami Cloud 这一无需 Cookie 的轻量级分析平台，收集匿名的聚合指标（例如页面访问次数、来源域名、设备操作系统及国家级别的大致地理位置），不记录个人真实身份，亦不跨外部站点追踪用户。此外，我们使用 Microsoft Clarity 获取匿名的导航遥测、点击热力图及滚动深度以排查界面缺陷，期间系统会自动遮蔽键盘按键记录及敏感文本输入框。'
    },
    {
      h: 'Cookie 与浏览器本地存储',
      p: 'Lade Stack 不使用任何持久性广告追踪 Cookie、跨站信标（Web Beacon）或强制身份认证 Cookie。为了在多次访问之间保留您的界面偏好，我们使用标准的浏览器客户端本地存储（LocalStorage）。具体而言，我们使用 "ladestack-theme" 键保存您的外观模式选择（深色或浅色模式），使用 "ladestack-lang" 键保存您的界面语言偏好。这些键值对仅驻留在您的设备本地，绝不会上传至外部服务器。您可以随时通过浏览器设置清除这些数据，且不会影响对工具基础功能的正常使用。'
    },
    {
      h: '浏览器内文件与文档处理',
      p: 'Lade Stack 生产力工具的一大核心架构特色在于 100% 纯客户端本地运行。像 LS PDF Tools 和 LS Image Studio 这类应用，完全利用现代 WebAssembly 二进制模块与 HTML5 Canvas API 在您浏览器的本地内存中运行。您的文档、PDF、图像和代码源文件从不上传、传输或保存在我们的服务器上。处理完成后，内存缓冲区和对象 URL 会在您的设备端被立即销毁（URL.revokeObjectURL），确保您的敏感文件享有绝对的隐私安全。'
    },
    {
      h: 'CodeEnhance AI 与代码输入处理',
      p: '当您将代码片段提交给 CodeEnhance AI 进行语法检查、代码重构或结构解释时，数据会经由端到端加密连接（TLS 1.3）安全传输。请求仅在服务器的瞬时内存中处理数秒，完成结果生成并将实时差异（diff）返回给您的编辑器后即告终止。我们不会将您的代码持久化存储到磁盘，不保留专有源代码的历史日志，也绝不会使用您提交的代码来训练、微调或优化底层机器学习模型。'
    },
    {
      h: '我们如何使用收集的信息',
      p: '我们收集的所有信息严格用于正当的运营支持：在我们承诺的 24 小时反馈窗口内回复技术咨询与支持请求；发送您订阅的产品动态；监控平台正常运行时间与边缘 CDN 交付性能；定位软件漏洞与不同浏览器间的兼容性问题；以及防范恶意滥用和分布式拒绝服务（DDoS）攻击。我们不进行任何用户画像分析、自动化信用与就业评估，亦不从事任何个人数据的商业转售。'
    },
    {
      h: '第三方服务商与数据共享',
      p: '我们仅与维持平台正常运行所必需的受信任基础设施提供商共享有限技术数据：Formspree（用于安全接收并转发联系表单信息）、Umami Cloud（用于注重隐私的聚合访问量统计）、Microsoft Clarity（用于排查前端故障的匿名热力图与性能诊断）以及全球 CDN 边缘节点网络（用于以超低延迟分发预渲染静态资源）。所有提供商均受严格的服务条款约束，严禁出于自身商业目的利用您的数据。'
    },
    {
      h: '数据留存与删除',
      p: '我们仅在实现本政策所述目的所需的必要期限内留存个人信息。技术支持往来信件与联系记录仅保留至问题完全解决并具备故障排查上下文所需的合理时长。浏览器本地数据（LocalStorage 偏好与工具临时草稿）会一直保留在您的设备中，直到您主动清理浏览器缓存或站点数据。您可以随时发送邮件至 admin@ladestack.in，要求彻底删除历史联络记录。'
    },
    {
      h: '您的隐私权利与数据可携性',
      p: '无论您身处何地，均有权向我们申请查阅关于您的沟通记录，要求更正不准确的信息，或申请删除相关联络数据。此外，对于在 Lade Stack 工具中处理的所有源代码、文档、简历和图片，您拥有完整且无条件的最终所有权。我们的应用通过直接文件下载、系统剪贴板复制以及规范的 JSON 项目导出，提供即时的数据可迁移性，绝不添加任何水印，亦不存在任何平台技术锁定。'
    },
    {
      h: '技术安全防护与用户安全倡议',
      p: '我们在工程层面部署了严密的安全防范措施：全站强制 HTTPS 传输加密、严格的内容安全策略（CSP）、用于代码实时预览的沙盒化 iframe 隔离环境（有效阻止跨站脚本攻击 XSS）以及自动化的依赖漏洞修复。然而，任何互联网传输都无法做到绝对不可攻破。我们建议开发者在使用在线工具时保持良好的安全习惯：请勿在任何公开工具中输入生产环境密钥、私有加密私钥或未脱敏的数据库密码。若发现潜在安全隐患，请直接联系 admin@ladestack.in。'
    },
    {
      h: '未成年人隐私、政策更新与联系方式',
      p: 'Lade Stack 面向广大公众、软件开发者、学习者与创作者开放；我们绝不主动收集 13 周岁以下儿童的个人信息。随着新工具的发布、系统架构的演进或法规标准的更新，我们可能会适时修订本隐私政策，最新修改以页面顶部的“最后更新”日期为准。若您对本隐私政策或个人信息处理有任何疑问、意见或诉求，欢迎通过邮件联络 Girish Lade：admin@ladestack.in（印度孟买）。'
    }
  ],
  terms: [
